...
class SelectorComputeMixin:
    ...
    def _apply_replay_refresh_multi_source_selector(
        self,
        plan: Any,
        *,
        phase: str,
    ) -> Optional[Tuple[SelectorResult, ...]]:
        """Run replay-refresh selector over descriptor-adjacent groups.

        Writer groups remain the public contract.  Adjacent groups backed by
        the same capture-score storage are selected in one batched call and
        then sliced back into per-writer-group results.  Groups that do not
        share storage keep the existing one-call-per-group path.
        """
        from patches.refresh_runtime.producer_workspace import (
            slice_selector_result_for_replay_group,
        )

        def _capture_storage_family_key(group: Tuple[Any, ...]) -> Tuple[Any, ...] | None:
            if not group:
                return None
            capture_scores = getattr(group[0], "capture_scores", None)
            if not isinstance(capture_scores, torch.Tensor):
                return None
            try:
                storage_ptr = int(capture_scores.untyped_storage().data_ptr())
            except Exception:
                storage_ptr = int(capture_scores.data_ptr())
            if storage_ptr == 0:
                return None
            return (
        ...
        def _slot_list_for_group(group: Tuple[Any, ...]) -> Sequence[int]:
            if not group:
                return tuple()
            slot_list = getattr(group[0], "slot_list", tuple())
            return tuple(int(slot) for slot in (slot_list or tuple()))

        groups = tuple(getattr(plan, "groups", ()) or ())
        if not groups:
            return tuple()
        payloads = tuple(getattr(plan, "payloads", ()) or ())
        group_spans = tuple(getattr(plan, "group_spans", ()) or ())
        if len(group_spans) != len(groups):
            raise RuntimeError(
                "replay refresh selector plan group_spans mismatch: "
                f"groups={len(groups)} spans={len(group_spans)}"
            )

        def _apply_with_private_selected_indices_out(
            payloads_for_run: Tuple[Any, ...],
        ) -> Optional[SelectorResult]:
            # Each returned result may outlive this helper until the async writer
            # consumes it. Do not let a later selector run overwrite selector
            # scratch that is still queued behind this run's stream work.
            ...
            try:
                return self._apply_alpha_selector_batched_fused(
                    payloads_for_run,
                    phase=str(phase),
                ...
                self._selector_key_norms_delta_buffer_override = key_norms_delta_prev
                self._log_f_scratch_workspace_override = log_f_workspace_prev
                self._selector_pipeline_workspace_override = (
        ...
        while group_index < len(groups):
            ...
            family_key = _capture_storage_family_key(group)
            run_end = group_index + 1
            if family_key is not None:
                while (
                    run_end < len(groups)
                    and _capture_storage_family_key(groups[run_end]) == family_key
                ):
                    run_end += 1
            if run_end - group_index <= 1:
                result = _apply_with_private_selected_indices_out(group)
                if result is None:
                    return None
            ...
            run_start_offset, _ = group_spans[group_index]
            last_offset, last_count = group_spans[run_end - 1]
            fused_count = int(last_offset) + int(last_count) - int(run_start_offset)
            fused_payloads = payloads[
                int(run_start_offset): int(run_start_offset) + int(fused_count)
            ]
            expected_payloads = tuple(
                payload for run_group in groups[group_index:run_end] for payload in run_group
            )
            if fused_payloads != expected_payloads:
                raise RuntimeError(
                    "replay refresh selector plan payload/group span mismatch"
                )
            result = _apply_with_private_selected_indices_out(fused_payloads)
            if result is None:
                return None
            for slice_index in range(group_index, run_end):
                span_start, span_count = group_spans[slice_index]
                results.append(
                    slice_selector_result_for_replay_group(
                        result,
                        layer_start=int(span_start) - int(run_start_offset),
                        layer_count=int(span_count),
                        slot_list=_slot_list_for_group(groups[slice_index]),
                    )
                )
            group_index = run_end
