...
_prepare_refresh_capture_payload = require_runtime_dep("_prepare_refresh_capture_payload")
...
def execute_refresh_post_kernel_impl(
    ...
    *,
    ...
    """Execute selector + compact rewrite after kernel based on step-wise refresh plan."""
    if step_context is None:
        raise RuntimeError(
            "refresh post-kernel missing step_context; "
            f"cache_key={int(key_cache.data_ptr())}"
        )
    step_epoch = step_context.epoch
    step_handle_id = step_context.step_handle_id
    step_handle_generation = step_context.step_handle_generation
    if refresh_slot_list is None:
        raise RuntimeError(
            "refresh post-kernel requires step_authority refresh_slot_list snapshot"
        )
    if not isinstance(refresh_slot_list, tuple):
        raise RuntimeError(
            "refresh post-kernel refresh_slot_list must be tuple from step_authority single-source snapshot"
        )
    if not refresh_slot_list:
        # refresh_slot_list is not None and (not refresh_slot_list)
        if refresh_slots or bootstrap_slots_set:
            raise RuntimeError(
                "refresh post-kernel has refresh/bootstrap intent but empty slot_list snapshot; "
                "refresh_slot_list is empty despite non-empty refresh/bootstrap intent"
            )
        if controller is not None:
            if step_handle_id > 0 and step_handle_generation > 0:
                controller._step_refresh_commit_note_post_kernel(
                    handle_id=step_handle_id,
                    handle_generation=step_handle_generation,
                )
            controller._step_profile_record_refresh_noop("empty_refresh_set")
        return
    if not refresh_slots and not bootstrap_slots_set:
        raise RuntimeError(
            "refresh post-kernel slot_list is non-empty while refresh/bootstrap intent is empty"
        )
    ...
    if step_authority is None:
        raise RuntimeError(
            "refresh post-kernel requires controller.step_authority single source"
        )
    if step_handle_id > 0 and step_handle_generation > 0:
        controller._step_refresh_commit_note_post_kernel(
            handle_id=step_handle_id,
    ...
    if step_handle_id <= 0 or step_handle_generation <= 0:
        raise RuntimeError(
            "refresh post-kernel missing step handle identity; "
            f"epoch={int(getattr(step_context, 'epoch', -1))}, "
    ...
    if profile_enabled and profile_stats is not None:
        profile_stats["selector"] += time.perf_counter() - t_selector

    if payload is None:
        payload_slot_count = len(refresh_slot_list)
        raise RuntimeError(
            "refresh payload build failed; refuse silent skip "
            f"(cache_key={int(key_cache.data_ptr())}, "
            f"epoch={int(getattr(step_context, 'epoch', -1))}, "
            f"slot_count={int(payload_slot_count)})"
        )
    ...
    # Refresh payload keeps layer-local Q/block-table references so the refresh
    # stream can build its payload without copying them on the main path.
    q_for_refresh = q
    q_is_sub = False
    ...
    controller._step_profile_record_refresh_payload()
    controller._enqueue_refresh_capture(
        SelectorBatchPayload(
            ...
            refresh_rows_long=refresh_rows_long,
            refresh_block_table_sub=refresh_block_table_sub,
            refresh_seq_lens_i32=refresh_seq_lens_i32,
            key_cache=key_cache,
            value_cache=value_cache,
            block_table=block_table,
            bootstrap_slots=bootstrap_slots_set,
            q=q_for_refresh,
            q_is_sub=q_is_sub,
            cu_seqlens_q=cu_seqlens_q,
            ...
            capture_handle_id=step_handle_id,
            capture_handle_generation=step_handle_generation,
            capture_epoch=step_epoch,
